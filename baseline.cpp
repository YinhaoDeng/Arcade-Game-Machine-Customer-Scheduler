...
#include <vector> //类似于array的储存方式，但是比array先进
...
public:
    ...
    int playing_since;   // 什么时候第一次开始运行的   [playing_since - arrival time = response time]
...
{
public:
    int event_time;  //在xxx时间点上
    int customer_id;  // each event involes exactly one customer   有谁要进来
...
int main(int argc, char *argv[])
{
    ...
    {
        //1  欢迎仪式 welcome newly arrived customers  从arrival_events进入到waiting queue里
        while (!arrival_events.empty() && (current_time == arrival_events[0].event_time))
        {
            queue.push_back(arrival_events[0].customer_id);  //进入waiting queue里
            arrival_events.pop_front(); //把这个人从arrival_events里删除
        }

        // 2 check if we need to take a customer off the machine
        if (current_id >= 0)
        {
            if (current_time == time_out)
            {
                ...
                if (customers[current_id].slots_remaining > 0)
                {
                    ...
                    queue.push_back(current_id); // 重新回到等待队列的末尾
                }
        ...
        // 3 选人的机制  if machine is empty, schedule a new customer
        if (current_id == -1)
        {
